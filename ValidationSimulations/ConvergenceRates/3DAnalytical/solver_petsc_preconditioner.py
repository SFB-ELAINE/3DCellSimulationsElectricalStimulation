...
fes.iterative_solver(
    ...
    "cg",
    "PETScPC",
    solver_kwargs={"maxsteps": 10000, "printrates": comm.rank == 0},
    preconditioner_kwargs={"pc_type": "gamg"},
)
...
# write results
if rank == 0:
    if not os.path.exists("results_petsc_pre"):
        os.makedirs("results_petsc_pre")
results = {"nels": nels, "ndofs": ndofs, "h1": h1errors, "l2": l2errors, "tmp": TMPerr}
df = pd.DataFrame(results)
filename = "results_petsc_pre/results_order"
if not curved:
    filename += "_uncurved"
