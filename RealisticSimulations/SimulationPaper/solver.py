...
for file in sorted(glob.glob("processed_meshes/*.vol.gz")):
...
dirichlet = {"ecm": "electrode_1|electrode_4"}
...
# set BC
bnd = {"electrode_1": 0.0, "electrode_4": 1.0}
...
for i, freq in enumerate(freqs):
    ...
    if freq == 1e3 or freq == 1e7 or freq == 1e8:
        fes.export_VTK(
            filename=f"field_{basename}_{int(freq / 1e3)}",
            sigma_dict=material_properties,
            subdivision=1,
            save_field=True,
        )
        fes.export_surface_jump_VTK(
            filename=f"TMP_{basename}_{int(freq / 1e3)}",
            subdivision=0,
            floatsize="single",
        )
    printonce("Z:", Z[i])
