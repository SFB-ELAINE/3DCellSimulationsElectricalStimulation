...
def plot_dilated_background(labeled_image, dilated_background, slice_index=None):
    ...
    # Original slice
    im0 = axes[0].imshow(original_slice, cmap="gray")
    axes[0].set_title("Original Slice")
    plt.colorbar(im0, ax=axes[0])

    # Dilated background mask slice
    im1 = axes[1].imshow(dilated_slice, cmap="gray")
...
image = skio.imread("Sample02_093_segCell.nii_relabel.tif")
...
tiff.imwrite("Sample02_093_marked_outer.tif",
    new_labeled_image.astype(np.uint8),
    compression="zlib",
)
np.save("marked_outer_093.npy", touching_labels)
...
# plot_dilated_background(image, dilated_background)
plot_touching_and_non_touching(image, touching_labels)
plot_touching_and_non_touching_zy(image, touching_labels)
# plot_dilated_background(image, dilated_background, slice_index=76)
...
# plot_dilated_background(image, dilated_background, slice_index=40)
