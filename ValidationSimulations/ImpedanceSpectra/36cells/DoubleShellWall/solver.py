...
mesh = load_mesh("doubleshellwallmesh.vol.gz", comm, curve_order=order)
printmaster("Done")
...
domains = ["ecm|pcm", "cytoplasm", "nucleoplasm"]
interface_info = {
    "ecm|pcm": "membrane",
    "cytoplasm": "membrane|nucleusmembrane",
    "nucleoplasm": "nucleusmembrane",
}
outer_inner_pairs = {
    "membrane": ("ecm|pcm", "cytoplasm"),
    "nucleusmembrane": ("cytoplasm", "nucleoplasm"),
}
dirichlet = {"ecm|pcm": "top|bottom"}
...
volume_ratio = ngsolve.Integrate(
    ...
    fes.mesh,
    definedon=fes.mesh.Materials("pcm|cytoplasm|nucleoplasm"),
)
...
interface_thickness = {"membrane": 7e-9, "nucleusmembrane": 40e-9}

conductivities = {"ecm": 1.0, "pcm": 0.01, "cytoplasm": 0.48, "nucleoplasm": 0.95}
permittivities = {"ecm": 80, "pcm": 20, "cytoplasm": 60, "nucleoplasm": 120}
interface_conductivities = {"membrane": 8.7e-6, "nucleusmembrane": 3e-3}
interface_permittivities = {"membrane": 5.8, "nucleusmembrane": 41}
...
filename = "results/results_double_shell_wall"
